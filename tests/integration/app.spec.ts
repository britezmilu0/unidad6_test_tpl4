...
describe('Todo API', () => {
  ...
  it('GET /todos/stats devuelve los totales', async () => {
    // Limpiar el servicio
    (app as any).todoService.clear();

    // Creamos datos de prueba
    const a = await request(app).post('/todos').send({ title: 'Tarea A' });
    const b = await request(app).post('/todos').send({ title: 'Tarea B' });
    const c = await request(app).post('/todos').send({ title: 'Tarea C' });

    // Marcamos uno como completado
    const toggled = await request(app).patch(`/todos/${b.body.id}/toggle`);
